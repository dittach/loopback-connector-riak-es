...
module.exports = function (riak, bucketName, query) {
	...
	return queries.map(function (query) {
		let bob = componentizeFacets(query).join(" AND ");
		return bob;
	}).join(" AND ");
	...
	function componentizeFacets(query) {
		...
		//console.log('facetComponents:', facetComponents);
	...
	function parseObjectPropertyQuery(propertyName, propertyQuery) {
		...
		//console.log('parseObjectPropertyQuery called, propertyName:', propertyName, ', propertyQuery:', propertyQuery);
		...
		switch (operator) {
			...
			case "neq":
				...
				break;
			case "regexp":
				return regexpQuery(propertyName, query);
				break;
		}
	}
	...
	function regexpQuery(propertyName, substring) {
		...
		*/
		//let regstr = '"regexp":{"' + propertyName + '": { "value": "' + new RegExp(substring).toString() + '" }}';
		//q: '"regexp":{"user_id": { "value": "/^o/" }}' }
		// (user_id://)
